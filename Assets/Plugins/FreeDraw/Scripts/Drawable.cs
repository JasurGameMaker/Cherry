...
//////////////////////////////////////////////////////////////////////////////
        ...
        void Update()
        {
            ...
            bool heldDown = Input.GetMouseButton(0) || Input.touchCount > 0;
            if (heldDown && !no_drawing_on_current_drag)
            {
                Vector3 position = Application.isMobilePlatform ? Input.GetTouch(0).position : Input.mousePosition;
                
                // Convert mouse coordinates to world coordinates
                Vector2 mouse_world_position = Camera.main.ScreenToWorldPoint(position);
            ...
            mouse_was_previously_held_down = heldDown;
        }
        ...
        public void Init(Texture2D tex)
        {
            ...
            Sprite initialSprite = CreateSprite(tex);
            spriteRenderer.sprite = initialSprite;
            drawable_sprite = initialSprite;
            drawable_texture = initialSprite.texture;
            GetComponent<BoxCollider2D>().size = initialSprite.bounds.size;
        }

        public void ResetTexture()
        {
            Sprite sprite = CreateSprite(_initialTex);
            spriteRenderer.sprite = sprite;
            drawable_sprite = sprite;
            drawable_texture = sprite.texture;
        }
        ...
        private static Sprite CreateSprite(Texture2D tex)
        {
            ...
            return Sprite.Create(texCopy, new Rect(0, 0, texCopy.width, texCopy.height), Vector2.one * 0.5f);
        }

        public float GetSpriteHeight() =>
            drawable_sprite.bounds.size.y;
    }
}
